    ...
    >
      <div v-if="message.type === 'assistant'" class="message-wrapper">
        ...
        <div class="flex items-center">
          <div>
            <div v-if="containsCodeBlock(message.content)">
              <div>{{ getTextWithoutCode(message.content) }}</div>
              <div class="code-block">
                <pre>
              <code
                ref="codeBlocks"
                :data-lang="getLanguage(message.content)"
              >{{ getCodeFromContent(message.content) }}</code>
            </pre>
              </div>
            </div>
            <div v-else>
...
export default {
  ...
  methods: {
    ...
    getTextWithoutCode(content) {
      return content.replace(/```[\s\S]*?```/, "").trim();
    },
    getLanguage(content) {
      const match = content.match(/```(.*?)\n/);
      return match ? match[1].trim() : "plaintext";
    },
  },
  updated() {
    this.$nextTick(() => {
      if (!this.$refs.codeBlocks) return;

      const codeBlocks = Array.isArray(this.$refs.codeBlocks)
        ? this.$refs.codeBlocks
        : [this.$refs.codeBlocks];
